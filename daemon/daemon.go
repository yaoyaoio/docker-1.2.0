...
func (daemon *Daemon) Install(eng *engine.Engine) error {
	...
	if err := daemon.Repositories().Install(eng); err != nil { //向eng对象注册和docker镜像相关的handler
		return err
	}
...
func NewDaemonFromDirectory(config *Config, eng *engine.Engine) (*Daemon, error) {
	...
	if config.Mtu == 0 { //MTU 容器网络接口的最大传输单元
	...
	log.Debugf("Creating images graph") //创建镜像graph
	...
	log.Debugf("Creating repository list") //创建镜像仓库列表
	repositories, err := graph.NewTagStore(path.Join(config.Root, "repositories-"+driver.String()), g)
	if err != nil {
		return nil, fmt.Errorf("Couldn't create Tag store: %s", err)
	}

	if !config.DisableNetwork { //配置网络 init_networkdriver = daemon/networkdriver/bridge/driver.go
	...
	daemon := &Daemon{
		repository:     daemonRepo,   //存在所有Docker容器信息的路径
		containers:     &contStore{s: make(map[string]*Container)}, //用于存储Docker容器信息的对象
		graph:          g,
		repositories:   repositories, //存储本机所有docker镜像repo信息的对象
		idIndex:        truncindex.NewTruncIndex([]string{}),
		sysInfo:        sysInfo, //系统功能信息
		volumes:        volumes, //管理宿主机逻辑卷的内容griver
		config:         config,
		containerGraph: graph,
		driver:         driver, //管理镜像的驱动grphdriver 默认为aufs类型
	...
	if err := daemon.restore(); err != nil { //加载已有Docker容器
		return nil, err
	}
